...
\newcommand{\mtip}[2]{\begin{Tip}{#1}{}#2\end{Tip}}
\newcommand{\mnote}[2]{\begin{Note}{#1}{}#2\end{Note}}
\newcommand{\mdef}[2]{\begin{Def}{#1}{}#2\end{Def}}
...
\DeclareDocumentCommand\curl{ g o d() }{ % Curl
	\IfNoValueTF{#1}{
		\IfNoValueTF{#2}{
      \IfNoValueTF{#3}{\text{curl}}{\text{curl}\quantity(#3)}
			}
      {\text{curl} \quantity[#2] \IfNoValueTF{#3}{}{(#3)}}
		}
    {\text{curl } #1 \IfNoValueTF{#2}{}{[#2]} \IfNoValueTF{#3}{}{(#3)}}
	}

\DeclareDocumentCommand\div{ g o d() }{ % Curl
	\IfNoValueTF{#1}{
		\IfNoValueTF{#2}{
      \IfNoValueTF{#3}{\text{div}}{\text{div}\quantity(#3)}
			}
      {\text{div} \quantity[#2] \IfNoValueTF{#3}{}{(#3)}}
		}
    {\text{div } #1 \IfNoValueTF{#2}{}{[#2]} \IfNoValueTF{#3}{}{(#3)}}
	}
% definitions
...
\newtcbtheorem[number within=section]{Def}{Def}
{%
	enhanced,
	breakable,
